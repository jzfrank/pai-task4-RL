...
def rnn(input_size, hidden_size, num_layers, activation, output_activation=nn.Identity):
    """The rnn layers"""
    return nn.RNN(
...
class MLPCategoricalActor(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        ...
        # self.num_layers = 1
        # self.logits_net = rnn(obs_dim, 16, self.num_layers, 'tanh')

    def _distribution(self, obs):
        """Takes the observation and outputs a distribution over actions."""
        logits = self.logits_net(obs)
        # logits = self.logits_net(obs, torch.Tensor([self.num_layers, obs.size()]))
...
class VPGBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma, lam):
        ...
        # advantage estimates
        # self.phi_buf = torch.zeros(size, dtype=torch.float32)
    ...
    def end_traj(self, last_val=0):
        ...
        self.phi_buf[path_slice] = torch.as_tensor(discount_cumsum(deltas, self.gamma * self.lam).copy())
        ...
        self.ret_buf[path_slice] = torch.as_tensor(discount_cumsum(rews, self.gamma)[:-1].copy())
        # self.val_buf[path_slice] = torch.as_tensor(discount_cumsum(vals, self.gamma)[:-1].copy())
    ...
    def get(self):
        ...
        self.phi_buf = normalize(self.phi_buf.reshape(1,-1)).reshape(-1)


        data = dict(obs=self.obs_buf, act=self.act_buf, ret=self.ret_buf,
                    phi=self.phi_buf, logp=self.logp_buf, val=self.val_buf)
...
class Agent:
    def __init__(self, env):
        ...
        self.ac = MLPActorCritic(hidden_sizes=[self.hid]*self.l, activation=nn.Tanh)
        # self.ac = MLPActorCritic(hidden_sizes=[32, 64, 32])
        ...
        self.pi_optimizer = AdamW(self.ac.pi.parameters(), lr=pi_lr)
        self.v_optimizer = AdamW(self.ac.v.parameters(), lr=vf_lr)
    ...
    def v_update(self, data):
        ...
        batch = ret.size()[0] // 2
        print("ret.size()[0] // batch", ret.size()[0] // batch)
        # for i in range(ret.size()[0] // batch):
        for i in range(4 * ret.size()[0] // batch):
            index = np.random.choice(ret.size()[0], batch, replace=True)
            start = i * batch
            # val = self.ac.v.forward(obs[start:start+batch])
            # diff = ret[start:start+batch] - val
            val = self.ac.v.forward(obs[index])
            diff = ret[index] - val
            loss = diff.dot(diff)
    ...
    def get_action(self, obs):
        ...
        action, value, log_prob = self.ac.step(obs)
        return action
